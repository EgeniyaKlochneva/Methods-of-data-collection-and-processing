...
r_headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (HTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36'}
url = 'https://hh.ru'
...
while True:
    ...
    for vacancy in vacancy_list:
        ...
        if vacancy_salary is not None:
            ...
            if vacancy_salary.find(
                    '–') > -1:  # метод возвр позицию элемента или -1, если ничего не найдено. - лучше копировать прям с сайта\
...
# with open('parser_results.txt', 'w') as f:
#     f.writelines(f'{vacancy_url} | {vacancy_name} | {employer_name} | {vacancy_min}-{vacancy_max}-{vacancy_cur}\n'
#                  f'{"-" * 20}\n' for i in all_vacancy)

with open('parser_results.csv', 'w', encoding='UTF-8') as f:
    # csv_writer = csv.writer(f_csv, delimiter=',')
    # reader = csv.reader(f, delimiter=';')
    f.writelines(f'{row}\n' for row in all_vacancy)
